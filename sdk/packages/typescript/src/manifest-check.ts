 ...
 */
export function warnIfFlagged(operationName: string): void {
  const entry = embeddedManifest[operationName];
  if (!entry || warned.has(operationName)) return;
  if (entry.deprecated) {
    ...
    warned.add(operationName);
    console.warn(
      `[whodb] ${operationName}'s behavior changed in this platform release — results may differ from previous SDK versions.${entry.note ? ` ${entry.note}` : ''}`,
    );
  }
}
