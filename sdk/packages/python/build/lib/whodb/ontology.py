...
def _to_record_inputs(values: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for key, value in values.items():
        if value is None:
            ...
            # Compact separators: all SDK languages emit the same wire bytes.
...
class OntologyHandle:
    ...
    def capabilities(self, record_key: Any | None = None) -> dict:
    ...
    def action_executions(self, record_key: Any, limit: int = 50) -> list[dict]:
