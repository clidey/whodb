...
func collectSourceFieldValues(fields []platform.SourceConnectionField, explicitValues map[string]string, advanced map[string]string, prompt sourceFieldPrompt) (map[string]string, map[string]string, error) {
	...
	for key, value := range explicitValues {
		...
		if !ok {
			return nil, nil, fmt.Errorf("source type does not define connection field %q; run whodb sources fields <source-type> to list valid fields", key)
		}
	...
	for _, field := range fields {
		...
		if (!ok || strings.TrimSpace(value) == "") && field.Required {
			if prompt == nil || !isInteractiveInput() {
				return nil, nil, fmt.Errorf("source field %s is required; pass --field %s=value or run whodb sources fields <source-type> to inspect required fields", field.Key, field.Key)
			}
		...
		if field.Required && strings.TrimSpace(value) == "" {
			return nil, nil, fmt.Errorf("source field %s is required; run whodb sources fields <source-type> to inspect required fields", field.Key)
		}
		if ok {
