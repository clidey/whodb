...
import { buildTableSortInput, buildTableWhereInput } from './query-inputs'
...
export function useDataQuery(params: UseDataQueryParams): { state: DataQueryState; actions: DataQueryActions } {
  ...
  const handleSubmitRequest = useCallback(async (overridePageOffset?: number) => {
    ...
    const sort = buildTableSortInput(sortColumn, sortDirection)
    const where = buildTableWhereInput(currentFilters, data?.columnTypes)
