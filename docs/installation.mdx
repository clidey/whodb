...
| `WHODB_LOG_FORMAT` | Log output format: `json` | `text` |
| `WHODB_LOG_FILE` | Redirect all non-HTTP logs to a file instead of stdout. Use `default` for `/var/log/whodb/whodb.log`, or specify a custom path | unset (stdout) |
| `WHODB_ACCESS_LOG_FILE` | Redirect all HTTP access logs to a dedicated file. Use `default` for `/var/log/whodb/whodb.access.log`, or specify a custom path | unset (disabled) |
