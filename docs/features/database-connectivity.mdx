...
Set the maximum time (in seconds) to wait when establishing a connection. Default is 90 seconds.
</Accordion>
</AccordionGroup>

## Advanced Options Reference

These options are available through the "Advanced Options" panel on the login page. When configuring connections via [environment variables](/installation#database-connection-profiles), these are the keys supported in the `advanced` field.

### SSL/TLS Options

Available for all database types (PostgreSQL, MySQL, MariaDB, ClickHouse, MongoDB, Redis, Elasticsearch).
...
<Info>
For each certificate, use either the `Content` variant (inline PEM string) or the `Path` variant (file path), not both. The `Path` variants are only available for preconfigured connection profiles (environment variables); the login form uses the `Content` variants instead.
</Info>
...
|-----|---------|-----------|-------------|
| `Port` | varies | All databases | Connection port (default: 5432 for Postgres, 3306 for MySQL, etc.) |
| `Connection Timeout` | `90` | SQL databases | Connection timeout in seconds |

### MySQL / MariaDB Options

| Key | Default | Description |
|-----|---------|-------------|
| `Parse Time` | `True` | Parse time values into Go `time.Time` |
| `Loc` | `UTC` | Time zone location for time parsing |
| `Allow clear text passwords` | `0` | Allow cleartext password authentication |

### ClickHouse Options

| Key | Default | Description |
|-----|---------|-------------|
| `HTTP Protocol` | `disable` | Use HTTP protocol instead of native |
| `Readonly` | `disable` | Enable read-only mode |
...
| Key | Default | Description |
|-----|---------|-------------|
| `URL Params` | | Additional URL query parameters appended to the connection string |
| `DNS Enabled` | `false` | Use `mongodb+srv://` connection scheme |

### Environment Variable Example

```bash
export WHODB_MYSQL_1='{
  "alias": "prod-mysql",
  "host": "db.example.com",
  "user": "app",
  "password": "secret",
