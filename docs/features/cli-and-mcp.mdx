...
Running `whodb` with no subcommand opens the interactive TUI.
...
whodb schemas --connection local-postgres --format json
whodb tables --connection local-postgres --schema public --format json
whodb columns --connection local-postgres --schema public --table users --format json

# Run a query
whodb query "SELECT * FROM users LIMIT 10" --connection local-postgres --format table

# Stream query rows
whodb query "SELECT * FROM events" --connection local-postgres --format ndjson --stream

# Export table data or query results
whodb export --connection local-postgres --table users --format csv --output users.csv
whodb export --connection local-postgres --query "SELECT * FROM users" --output users.xlsx

# Import CSV or Excel
whodb import --connection local-postgres --file users.csv --table users
whodb import --connection local-postgres --file orders.xlsx --table orders --sheet Q1 --mode overwrite

# Analyze before generating mock data
whodb mock-data --connection local-postgres --table orders --rows 50 --analyze

# Generate mock data after confirmation
whodb mock-data --connection local-postgres --table orders --rows 50 --yes

# Run quality checks
whodb audit --connection local-postgres --schema public --format json

# Compare environments
whodb diff --from staging --to prod --schema public --format json

# Render relationship metadata
whodb erd --connection local-postgres --schema public --format json

# Emit agent-facing CLI and MCP metadata
whodb agent schema --format json

# Diagnose a connection without exposing secrets
whodb doctor --connection local-postgres --schema public --format json

# Preview a built-in workflow before running it
whodb runbooks run schema-audit --connection local-postgres --dry-run
...
whodb tables --connection ci-postgres --format json
```
...
whodb runbooks run connection-doctor --connection local-postgres --dry-run
whodb runbooks run schema-audit --connection local-postgres --schema public --format json
...
whodb mcp serve --allow-write --allow-drop
```
...
```bash
whodb mcp serve --transport=http --host=0.0.0.0 --port=3000 --auth-token your-secret-token
```
...
See `.agents/docs/hosted-platform-cli.md` in the repository for the full hosted
platform CLI and MCP workflow.
...
Use `whodb` for supported scripting and pipeline workflows.
</Card>
