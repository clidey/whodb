...
The data grid shows the current page of rows in a familiar spreadsheet-like format. Use page size, pagination, and WHERE conditions to control how much data is loaded.
</Step>
...
Executed queries are available in the current cell's history.

![Query History](/images/33-scratchpad-query-history.png)

For important analysis SQL, keep a copy in your team's normal documentation or repository. If you work in the CLI, use CLI bookmarks for named reusable queries.
</Step>
</Steps>
...
<Steps>
<Step title="Choose The Export Entry Point">
Choose the entry point that matches what you want to download:

![Export Dialog](/images/20-data-view-export-dialog.png)

- **Export All**: Export from the table or header context menu
- **Export Selected**: Select loaded rows first, then export selected rows
- **Filtered export**: Apply WHERE conditions before exporting

For analysis, apply the WHERE conditions first so the exported rows match the dataset you reviewed.
</Step>
