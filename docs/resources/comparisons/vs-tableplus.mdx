...
| **Runtime** | Go server + browser UI | Native desktop app |
| **Resource Profile** | Lightweight shared deployment | Local desktop process |
| **Platform** | Web-based (any browser) | Native (Mac, Windows, Linux) |
| **Installation** | No installation | Download + install |
| **Offline Access** | Requires browser/server connectivity | Yes |
| **Multi-Database** | Yes (broad CE catalog) | Yes (15+) |
| **Database Support** | Current CE catalog | Good coverage |
| **UI Modern** | Yes | Yes |
| **Learning Curve** | Very low | Very low |
| **Team Features** | Shared deployment with database-side controls | Limited |
| **Collaboration** | Central server access and shared workflows | Individual licenses |
| **Export Options** | CSV/Excel; JSON Lines for document-style records when supported | Multiple formats |
...
- Team (10): $0/month for CE, plus your hosting costs
- Larger teams: $0/month for CE, plus hosting, operations, and support needs
...
|--------|-------|-----------|
| Opening the tool | Open the hosted/local WhoDB URL | Launch the desktop app |
| Connection context | Shared server deployment or local instance | Local desktop connection profile |
| Team access | One deployment can serve multiple users | Installed per user |

TablePlus is very fast for individual desktop workflows. WhoDB is stronger when you want browser access, Docker deployment, and a shared app surface.
...
The current CE catalog includes PostgreSQL, MySQL, MariaDB, CockroachDB, SQLite3, MongoDB, Redis, ElasticSearch, ClickHouse, DuckDB, Memcached, TiDB, Valkey, Dragonfly, OpenSearch, YugabyteDB, QuestDB, and FerretDB, plus compatible managed variants surfaced through provider discovery.
...
| **Search** | Loaded-row search plus WHERE filters | Limited |
| **Favorites** | Saved profiles | Yes |
...
| **Query History** | Per-cell web history; CLI history search | Recent queries |
| **Saved Queries** | CLI bookmarks | Yes |
...
|---------|:----:|:--------:|
| **CSV** | Yes | Yes |
| **JSON** | JSON Lines for document-style records when supported | Yes |
| **SQL** | Use native database tools | Yes |
| **Excel** | Yes | Yes |
| **XML** | No | Yes |
| **PDF** | No | Yes |
...
| **Index Management** | Use database-native tools | Yes |
| **Constraint Management** | Use database-native tools | Yes |
| **View Management** | Use database-native tools | Yes |
| **Backup/Restore** | Native database tools recommended | Limited |
</Accordion>
...
Built-in realistic test data generation for supported column types. Useful for testing workflows. TablePlus doesn't have this built in.
</Card>

<Card title="Interactive Schema Graph">
Visual schema exploration with relationship lines, node cards, and data actions. TablePlus has basic visualization only.
</Card>
...
Works completely offline after installation. WhoDB needs connectivity between the browser, the WhoDB server, and the database.
</Card>
...
**Time to Productivity:** Short for users familiar with desktop database tools
...
|---------|:----:|:--------:|
| **SSL/TLS** | Yes | Yes |
| **SSH / Bastion Workflow** | External tunnel in web, built-in in CLI | Yes |
...
| **LDAP** | Use reverse proxy or external auth layer | No |
...
**Verdict:** Both can be deployed securely. TablePlus has system keychain integration; WhoDB CE usually relies on browser/server profiles, environment-defined profiles, and the authentication layer in front of the app.
...
**Result:** WhoDB is usually easier to standardize for teams because it can run as a shared deployment.

### Scenario 3: Large Team (50+ developers)
...
- Easy IT management
- Scale by sizing the web service, database connectivity, and reverse proxy appropriately
- Consistent across organization
- **Cost: $0 for CE, plus hosting and operations**
...
- Walk the team through the interface and existing workflows
...
- Copy important SQL out of Scratchpad pages or CLI bookmarks
...
TablePlus is the solution for offline-first work. WhoDB needs connectivity between your browser, the WhoDB server, and the database. That can be local or self-hosted, but it is not an offline desktop workflow.
</Accordion>
...
WhoDB fits central deployment and shared operational workflows. TablePlus is more individual-focused. For solo work, TablePlus remains a good native desktop option.
</Accordion>
