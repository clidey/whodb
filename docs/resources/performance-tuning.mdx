...
### Connection Management In The Current CE Build
...
These values are internal to the server and are not configured with `DB_MAX_CONNECTIONS`, `DB_MAX_IDLE_CONNECTIONS`, or related web env vars in the current build.
...
- Monitor actual connection usage on the database server
- Don't set max connections higher than the database allows
...
### Memory And Result Handling In CE
