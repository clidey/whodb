...
description: "Use WhoDB and the whodb in CI/CD pipelines for database validation, exports, imports, and data-quality checks"
---
...
- Run `whodb` for scriptable checks such as listing schemas, listing tables, running validation queries, exporting rows, importing CSV or Excel fixtures, and running data-quality audits.
...
<Note>
`GET /health` returns plain text `ok` when the WhoDB server is running. Validate database state separately with a database client or `whodb query`.
</Note>
...
Use native SQL seed files or `whodb import` for CSV and Excel fixtures.
</Card>
...
npx @clidey/whodb schemas --connection ci-postgres --format json
npx @clidey/whodb tables --connection ci-postgres --schema public --format json

# Run validation SQL
npx @clidey/whodb query "SELECT COUNT(*) FROM users" --connection ci-postgres --format json

# Export a table for artifacts or downstream checks
npx @clidey/whodb export --connection ci-postgres --table users --format csv --output users.csv

# Import CSV or Excel fixtures
npx @clidey/whodb import --connection ci-postgres --file test/fixtures/users.csv --table users

# Run data-quality checks
npx @clidey/whodb audit --connection ci-postgres --schema public --format json
```
