...
| `WHODB_ACCESS_LOG_FILE` | Write HTTP access logs to a dedicated file. Use `default` for `/var/log/whodb/whodb.access.log`, or specify a custom path | unset (disabled) |

<Note>
Log files are created with `0600` permissions (owner-only read/write) and directories with `0750`. WhoDB refuses to write to a path that is a symlink.
</Note>
