...
| `WHODB_LOG_FILE` | Redirect all non-HTTP logs to a file. Use `default` for `/var/log/whodb/whodb.log`, or specify a custom path | unset (stdout) | `default`, `/var/log/whodb/whodb.log` |
| `WHODB_ACCESS_LOG_FILE` | Write HTTP access logs to a dedicated file. Use `default` for `/var/log/whodb/whodb.access.log`, or specify a custom path | unset (disabled) | `default`, `/var/log/whodb/access.log` |
...
WhoDB applies restrictive permissions to log files and directories:

- **Directories** are created with `0750` (owner rwx, group rx, others none)
- **Files** are created with `0600` (owner rw only) — logs may contain query content or connection info
- **Symlink protection** — WhoDB refuses to open a log file path that is a symlink, preventing symlink-based attacks where a malicious link redirects log writes to a sensitive file

<Note>
The `default` log paths write to `/var/log/whodb/`, which requires root access. Docker containers run as root by default, so this works out of the box. If running WhoDB as a non-root user, specify a custom path instead (e.g., `WHODB_LOG_FILE=/home/appuser/logs/whodb.log`).
</Note>
