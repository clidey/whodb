...
A set of rules and tools for building software applications. WhoDB uses backend endpoints internally for its own web UI, but those endpoints are not documented as a supported public integration API. For scripting and automation, use `whodb` or database-native clients.
