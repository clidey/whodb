...
A set of rules and tools for building software applications. WhoDB uses backend endpoints internally for its own web UI, but those endpoints are not documented as a supported public integration API. For scripting and automation, use `whodb-cli` or database-native clients.
...
**Backup**
A complete copy of database data created for recovery purposes. Use database-native tools for full backups; WhoDB export is for grid/query-result data extracts.
...
**Export**
The process of extracting data from WhoDB. Current CE formats are CSV, Excel, and JSON Lines for records from sources like MongoDB when supported.

**Explore View**
The Storage Unit data route used to view rows. For structure and metadata, use **Describe** on a Storage Unit card.
...
**Format Query**
Readable SQL style with consistent indentation and line breaks. The CLI TUI includes SQL formatting; the current CE web Scratchpad focuses on syntax highlighting and execution.
...
**GraphQL**
A query language and runtime for APIs (Application Programming Interface) used in WhoDB for most backend communication, providing strongly-typed and flexible data querying. AI chat streaming and file export/import use dedicated HTTP endpoints.
...
**Hash Function**
A cryptographic function that maps data to a fixed-size digest, commonly used for password hashing and data integrity verification. WhoDB protects connection passwords with AES-256-GCM encryption server-side, and with the OS keyring in the desktop app.
...
**JSON Export**
For records from sources like MongoDB, WhoDB can export newline-delimited JSON when the source supports it. Table exports use CSV and Excel.
...
**SSH (Secure Shell)**
A secure protocol for remote command execution. In this codebase, SSH tunneling is handled either by an external tunnel you create yourself or by the CLI tooling rather than by a dedicated web login form control.
...
**Storage Unit**
WhoDB's terminology for a data object within a connected source — a table, view, collection, key, or similar object exposed by a connector — that you can explore and interact with.
...
- **Storage Units**: Source object browser for tables, collections, views, and equivalent objects
- **Describe**: Storage Unit metadata and column inspection
- **Chat Feature**: AI-powered query assistant
- **Mock Data**: Generated test data
- **Export**: Data download functionality
- **Selected Row Actions**: Operations on selected loaded rows where supported
- **Keyboard Shortcuts**: Quick action commands
</Accordion>

<Accordion title="WhoDB Packaging">
- **Community Edition**: Free, open-source version
- **Storage Unit**: Source object such as a table, view, collection, key, or similar object exposed by a connector
- **Connection Profile**: Saved database credentials
</Accordion>

<Accordion title="WhoDB Operations">
- **Edit Row**: Side-sheet row update flow from the data grid
- **Export**: CSV, Excel, or supported document-row download
