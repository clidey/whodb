...
An inline confirmation alert appears in the chat. Click **Show Query** to see:
...
Always check the generated INSERT statement in the confirmation prompt (via **Show Query**) to ensure:
...
Verify the "Action Executed" message, then run a follow-up SELECT to confirm the intended records changed.
</Step>
</Steps>
...
If updating one user, expect 1 row affected. If the WHERE clause in the confirmation could match more, cancel and refine your query.
</Accordion>
...
The assistant creates the DELETE statement and holds it for confirmation — nothing executes yet.
</Step>
<Step title="Review the Inline Confirmation">
An alert appears in the chat warning that the operation will modify your database. Click **Show Query** to see:
- The DELETE statement
- Which table will be affected
- The WHERE clause indicating which records will be removed
</Step>
<Step title="Confirm or Cancel">
Click the Confirm button to execute the DELETE, or Cancel to abort.
</Step>
<Step title="Verify Deletion">
Check the "Action Executed" message, then run a follow-up SELECT to confirm the records are gone.
</Step>
</Steps>
...
The inline confirmation is your final checkpoint before data modification. Understanding what to look for helps ensure safe operations.
...
Click **Show Query** to see the exact SQL that will execute, including:
- Target table name
...
<Step title="Check the Operation Name">
The success message shows which operation ran (for example, "DELETE" or "UPDATE").
</Step>
<Step title="Query the Modified Data">
Run a SELECT query to verify the changes and see how many rows were actually affected:
...
Plan recovery before making data modifications. These strategies help you recover from mistakes.
...
Always review the confirmation and the SQL behind **Show Query** before clicking confirm.
...
**Symptom**: Modifying many records takes a long time or strains the database.

**Solutions**:
- Break the operation into smaller batches: update or delete a limited subset at a time
- Use the Scratchpad to run batched statements with transaction control
- Consider database-level bulk operations for very large datasets
- Schedule large modifications during low-traffic periods
</Accordion>
</AccordionGroup>
...
Modifications are associated with the database user credentials used, so database-level logs can attribute changes.
</Step>
<Step title="Database-Level Auditing">
Enable your database's audit or query logging for comprehensive tracking of executed statements.
</Step>
<Step title="Application Logs">
Set `WHODB_LOG_FILE` to capture WhoDB's application logs. Note this is a general application log, not a per-query audit trail.
</Step>
</Steps>
...
<Check>
**Safe Modifications**: Every data modification and schema change (INSERT, UPDATE, DELETE, CREATE, ALTER, DROP) requires explicit confirmation before execution
</Check>
