...
- Feature usage stats
- Error rate trends
Save in Scratchpad for your own reuse — Scratchpad pages are stored per-browser, so share the SQL itself (in docs or version control) for team reference
</Step>
...
Review database-native audit logs and infrastructure logs for recent access
</Step>
<Step title="Transfer Shared Materials">
Move any shared SQL files, runbooks, or exported reports to the owning team
</Step>
...
4. Reduce result sizes or lower the default page size in [Settings](/features/settings)
5. Consider database optimization, database-side pooling, or scaling
</Accordion>
...
- Database-side role-based access control
- Regular backups
...
- Database-side pooling or proxies where needed
