...
Build analysis step-by-step. Each cell runs as an independent, self-contained statement, so include everything a cell needs — including its CTEs — within that cell:

![Multiple Cells](/images/48-scratchpad-multiple-cells-results.png)

**Cell 1**: Inspect the cleaned, filtered data
```sql
...
**Cell 2**: Aggregate into meaningful metrics (repeat the filter in a CTE)
```sql
...
**Cell 3**: Format for presentation (chain the CTEs in one statement)
```sql
...
This step-by-step approach is easier to debug and modify: earlier cells validate each stage, and the final cell contains the complete, self-contained query.
</Step>
...
- Load and modify previous SQL in the current editor
...
Options:
- **All**: Export from the table or header context menu
- **Selected**: Select loaded rows first, then export selected rows
- **Filtered**: Apply WHERE conditions before exporting
</Step>
...
JSON Lines is available for records from sources like MongoDB when supported.
</Accordion>
</AccordionGroup>
</Step>
...
- **Format**: CSV or Excel (JSON Lines is offered for NoSQL-style sources)
- **CSV delimiter**: Comma (standard), semicolon (European), or pipe (data with commas)

To export a subset of columns, select only those columns in your query before exporting.
</Step>
...
<Card title="Use Query History" icon="history">
