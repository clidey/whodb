...
- **Refresh manually with Cmd/Ctrl+R** when your test framework modifies the database
</Step>
...
Open mock data generation from the table/header context menu, or press `Cmd/Ctrl + Shift + G` while the table has focus:
...
Generate several 200-row batches, import fixture files, or use database-native scripts when you need thousands of rows.
```
...
Create larger fixtures for stress testing:
...
Use imports or database-native scripts for these larger datasets; the in-app mock data generator is capped at 200 rows per run.
</Step>
</Steps>
