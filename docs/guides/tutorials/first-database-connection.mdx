...
<Card title="Additional Engines" icon="plus">
...
**Certificate Fields**: If your database requires client certificates, provide:
- SSL CA Content: The CA certificate used to verify the server
- SSL Client Cert Content: Your client certificate
- SSL Client Key Content: Your client private key
- SSL Server Name: Optional hostname override for certificate validation
...
<Accordion title="Database-Specific Fields">
Some database types expose a slightly different set of advanced fields. Common examples include file paths for embedded databases, schema defaults, or provider-specific connection options.

<Note>
If you are unsure which advanced fields matter for your database, start with the minimum required host, port, username, password, and database values, then add SSL/TLS only when your provider or administrator asks for it.
</Note>
</Accordion>
<Accordion title="Private Networks And Bastions">
WhoDB needs a network path from the machine running the app to the database.

- If the database is directly reachable, connect to it normally with its host and port.
- If it sits behind a bastion or VPN, use that private network path first.
- If you need an SSH tunnel, create it externally or use the [CLI & MCP](/features/cli-and-mcp) tooling, which includes built-in SSH tunneling.
</Accordion>
</AccordionGroup>
...
2. **Certificate content is incorrect**
   - Re-paste the certificate content or re-upload the certificate file in the SSL options
   - Make sure you pasted the full PEM block, including the BEGIN/END lines
