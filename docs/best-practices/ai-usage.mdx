...
Every model has a context limit, and quality degrades as you approach it. Limits vary widely by model — check your provider's documentation.
...
Hover over query results, click the ellipsis (...) button, and select **Move to Scratchpad**.
...
Open the result's ellipsis (...) menu and select **Show Code** to see the generated SQL.
...
Use database-level audit logging to record executed queries. Per-query audit trails with user attribution are an Enterprise capability. You can set `WHODB_LOG_FILE` to capture WhoDB's application logs, but this is not a query audit trail.
