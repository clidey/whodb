...
<Accordion title="Clear Profile Naming for Safety">
Use unambiguous profile names to prevent accidental production operations:

- Prefix names with the environment: `prod-`, `staging-`, `dev-`
- Include the access level where relevant: `prod-postgres-readonly`
- Always verify the profile name before executing modifications
- Train the team on the naming convention
</Accordion>
