...
export interface SourceConnectionTraitsDescriptor {
	...
	hostInputMode: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["connection"]["hostInputMode"];
	hostInputUrlParser: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["connection"]["hostInputUrlParser"];
	supportsCustomCAContent: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["connection"]["supportsCustomCAContent"];
}
...
export interface SourcePresentationTraitsDescriptor {
	profileLabelStrategy: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["presentation"]["profileLabelStrategy"];
	schemaFidelity: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["presentation"]["schemaFidelity"];
}
...
export interface SourceQueryTraitsDescriptor {
	supportsAnalyze: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["query"]["supportsAnalyze"];
	supportsScripts: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["query"]["supportsScripts"];
	supportsStreaming: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["query"]["supportsStreaming"];
	supportsMultiStatement: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["query"]["supportsMultiStatement"];
	supportsSqlImport: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["query"]["supportsSqlImport"];
...
export interface SourceMockDataTraitsDescriptor {
	supportsRelationalDependencies: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["mockData"]["supportsRelationalDependencies"];
}
...
export interface SourceMetadataTraitsDescriptor {
	...
	graph: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["metadata"]["graph"];
	systemObjectFiltering: NonNullable<
		SourceTypesQuery["SourceTypes"][number]["traits"]
	>["metadata"]["systemObjectFiltering"];
}
...
function resolveIcon(sourceType: string, connector: string): ReactElement {
	...
	if (!icon && import.meta.env.DEV) {
		console.warn(
			`[source-types] No icon registered for source type "${sourceType}" or connector "${connector}". ` +
				`Add one to src/components/icons.tsx via registerIcons().`,
		);
	}
	return icon ?? createElement("span", { className: "w-6 h-6" });
}
...
export function sourceSupportsAction(
	...
	return (
		item?.contract?.ObjectTypes.some((objectType) =>
			objectType.Actions.includes(action),
		) ?? false
	);
}
 ...
 */
export function sourceDefaultObjectIsTabular(
	item: SourceTypeItem | undefined,
): boolean {
...
function decorateSourceType(item: BackendSourceType): SourceTypeItem {
	...
	const searchPathField = findConnectionField(
		item.connectionFields,
		"Search Path",
	);
	const browsePath = item.contract.BrowsePath;
	const objectTypes = item.contract.ObjectTypes;
	const defaultObjectType = objectTypes.find(
		(objectType) => objectType.Kind === item.contract.DefaultObjectKind,
	);
	...
	return {
		...
		usesSchemaForGraph:
			item.contract.GraphScopeKind === SourceObjectKind.Schema,
		usesDatabaseInsteadOfSchema:
			item.contract.GraphScopeKind === SourceObjectKind.Database ||
			(!browsePath.includes(SourceObjectKind.Schema) &&
				browsePath.includes(SourceObjectKind.Database)),
		supportsMockData: objectTypes.some((objectType) =>
			objectType.Actions.includes(SourceAction.GenerateMockData),
		),
...
function mergeSourceTypeOverride(
	...
	return {
		...
		traits: override.traits
			? {
					connection: {
						...(item.traits?.connection ?? {}),
						...override.traits.connection,
					},
					presentation: {
						...(item.traits?.presentation ?? {}),
						...override.traits.presentation,
					},
					query: { ...(item.traits?.query ?? {}), ...override.traits.query },
					mockData: {
						...(item.traits?.mockData ?? {}),
						...override.traits.mockData,
					},
					metadata: {
						...(item.traits?.metadata ?? {}),
						...override.traits.metadata,
					},
				}
			: item.traits,
		fields: override.fields
			? { ...item.fields, ...override.fields }
			: item.fields,
		requiredFields: override.requiredFields
			? { ...item.requiredFields, ...override.requiredFields }
			: item.requiredFields,
	};
}

function withRegisteredSourceTypes(
	baseTypes: SourceTypeItem[],
): SourceTypeItem[] {
...
export function findSourceObjectType(
	...
	return item.contract.ObjectTypes.find(
		(objectType) => objectType.Kind === kind,
	);
}
