...
export function verifyColumnTypes(fields, expectedColumns) {
    Object.entries(expectedColumns).forEach(([col, type]) => {
        ...
        expect(found, `Column ${col} should have type ${type}`).toBeTruthy();
    });
}
...
export function verifyMetadata(fields, metadata) {
    if (metadata.type) {
        expect(fields.some(([k, v]) => k === 'Type' && v === metadata.type)).toBeTruthy();
    }
    if (metadata.hasSize) {
    ...
    if (metadata.hasCount) {
        expect(fields.some(([k]) => k === 'Total Count:' || k === 'Total Count' || k === 'Count')).toBeTruthy();
    }
}
...
export function verifyScratchpadOutput(output, expectedColumns, expectedRows) {
    if (expectedColumns) {
        expect(output.columns).toEqual(expectedColumns);
    }
    if (expectedRows) {
        expect(output.rows.map(row => row.slice(0, -1))).toEqual(expectedRows);
    }
}
