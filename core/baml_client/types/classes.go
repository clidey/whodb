...
func (c *AgentAction) Decode(holder *cffi.CFFIValueClass, typeMap baml.TypeMap) {
	...
	for _, field := range holder.Fields {
		...
		case "action":
			c.Action = baml.Decode(valueHolder).Interface().(AgentActionType)
		...
		case "requires_confirmation":
			c.Requires_confirmation = baml.Decode(valueHolder).Bool()
...
func (c *ChatResponse) Decode(holder *cffi.CFFIValueClass, typeMap baml.TypeMap) {
	...
	for _, field := range holder.Fields {
		...
		case "type":
			c.Type = baml.Decode(valueHolder).Interface().(ChatMessageType)

		case "operation":
			c.Operation = baml.Decode(valueHolder).Interface().(*OperationType)
