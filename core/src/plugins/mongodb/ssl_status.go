...
func (p *MongoDBPlugin) GetSSLStatus(config *engine.PluginConfig) (*engine.SSLStatus, error) {
	log.Logger.Debug("[SSL] MongoDBPlugin.GetSSLStatus: checking configured SSL mode")
	...
	if sslConfig == nil || !sslConfig.IsEnabled() {
		log.Logger.Debug("[SSL] MongoDBPlugin.GetSSLStatus: SSL not configured or disabled")
	...
	log.Logger.Debugf("[SSL] MongoDBPlugin.GetSSLStatus: SSL enabled, mode=%s", sslConfig.Mode)
	return &engine.SSLStatus{
		IsEnabled: true,
