...
func (p *ElasticSearchPlugin) GetSSLStatus(config *engine.PluginConfig) (*engine.SSLStatus, error) {
	log.Logger.Debug("[SSL] ElasticSearchPlugin.GetSSLStatus: checking configured SSL mode")
	...
	if sslConfig == nil || !sslConfig.IsEnabled() {
		log.Logger.Debug("[SSL] ElasticSearchPlugin.GetSSLStatus: SSL not configured or disabled")
	...
	log.Logger.Debugf("[SSL] ElasticSearchPlugin.GetSSLStatus: SSL enabled, mode=%s", sslConfig.Mode)
	return &engine.SSLStatus{
		IsEnabled: true,
