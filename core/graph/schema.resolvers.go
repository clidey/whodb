...
func (r *mutationResolver) ImportPreview(ctx context.Context, file graphql.Upload, options model.ImportFileOptions, schema *string, storageUnit *string, useHeaderMapping *bool) (*model.ImportPreview, error) {
	...
	if *useHeaderMapping && !options.HasHeader {
		detail := importValidationHeaderMissing
		preview.ValidationError = &detail
		return preview, nil
	}
...
func (r *mutationResolver) ImportTableFile(ctx context.Context, input model.ImportFileInput) (*model.ImportResult, error) {
	...
	if err != nil {
		return importResult(false, importErrorTableConstraints), nil
	}
	...
	if allowAutoGenerated && (input.Options == nil || !input.Options.HasHeader) {
		return importResult(false, importValidationAutoGeneratedHeader), nil
	}

	mappings, err := resolveImportMappings(parsed.columns, input.Mapping, columns, constraints, allowAutoGenerated)
	if err != nil {
		return importResult(false, validationKeyFromError(err)), nil
	}
